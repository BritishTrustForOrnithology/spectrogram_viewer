...
class Actions():
  def __init__(self, parent):
    self.parent = parent #this allows referencing objects in the parent class, i.e. in one of the page classes
  ...
  def select_folder(self):
    ...
    if len(folder)>0:
      ...
      for filetype in filetypes:
        self.audio_files.extend([filename for filename in pathlib.Path(folder).rglob('*' + filetype)] )
  ...
  def open_audio_file(self, file):
    ...
    if self.duration.get() > self.maxduration:
      tk.messagebox.showerror(title='Error', message='File is too long!')
    #print(dur)
    if self.duration.get() <= self.maxduration:
  ...
  def create_spectrogram(self, x, sr):
    ...
    labs = np.unique(np.append(np.linspace(0,floor(self.duration.get()), floor(self.duration.get())+1), self.duration.get()))
    print(labs)
...
class Page_single(Page):
  def __init__(self, *args, **kwargs):
    ...
    #buttons 
    button_select_file = tk.Button(actionsframe, text="Select audio file", command = lambda: actions.select_file())
    ...
    label_filename = tk.Label(infoframe1, textvariable=actions.current_file, bg = 'white')
...
class Page_multiple(Page):
  def __init__(self, *args, **kwargs):
    ...
    button_select_folder = tk.Button(actionsframe, text="Select folder", command = lambda: actions.select_folder())
    ...
    label_folder = tk.Label(infoframe1, textvariable=actions.current_folder, bg = 'white')
